...
public class PlayerService {
	...
	public Page<PlayerDto> getAllWithFilterCategory(String pageNumber, String pageSize, String category) {
		final String categoryUpper = category.toUpperCase();
		...
		if (categories.contains(categoryUpper)) {

			LOG.info(String.format("Listing players by filter %s", categoryUpper));
			playerList = playerRepository.findAllWithFilter(categoryUpper, pageRequest);
			playerDto = playerList.stream()
					.map(player -> new PlayerDto(player.getName(), player.getLevel(),
							player.getSkills().get(0).getScore(),
							player.getSkills().stream().filter(s -> s.getSkillCategory().name().equals(categoryUpper))
									.collect(Collectors.toList())))
					.collect(Collectors.toList());
		} else {

			LOG.info("Listing players without filters");
	...
	/**
	 * Search a player by name, verify if the string is contained ou is exactly the same.(case insensitive)
	 * 
	 * @param playerName
	 * @param pageNumber
	 * @param pageSize
	...
	public List<Player> comparePlayers(Long playerAId, Long playerBId) {
		...
		List<Player> playerList = Arrays.asList(new Player[] { playerA, playerB });
